        ...
        for line in f:
            ...
            if (data['court']['name'] in state_high_list) and len(data['casebody']['data']['opinions']) > 0: #and len(data['casebody']['data']['opinions'][0]['text'].split()) > 50:
                ...
                try:
                    ...
                    ari_r = r_read_mets[1].r_repr()
                    rix_r = r_read_mets[35].r_repr()
                    Coleman_Liau_Grade_R = r_read_mets[9].r_repr()
                    Coleman_Liau_Short_R = r_read_mets[10].r_repr()
                    Danielson_Bryan_R = r_read_mets[14].r_repr()
                    Dickes_Steiwer_R = r_read_mets[16].r_repr()
                    ELF_R = r_read_mets[18].r_repr()
                    Farr_Jenkins_Paterson_R = r_read_mets[19].r_repr()
                    flesch_R = r_read_mets[20].r_repr()
                    flesh_kincaid_R = r_read_mets[22].r_repr()
                    FORCAST_R = r_read_mets[26].r_repr()
                    Fucks_R = r_read_mets[28].r_repr()
                    FOG_R = r_read_mets[23].r_repr()
                    Linsear_Write_R = r_read_mets[29].r_repr()
                    nWS_R = r_read_mets[31].r_repr()
                    SMOG_R = r_read_mets[37].r_repr()
                    Strain_R = r_read_mets[43].r_repr()
                    Wheeler_Smith_R = r_read_mets[46].r_repr()
...
#states_single_df_wide = states_single_df_wide.drop(['citations'], axis=1)
#old.shape
#states_single_df_wide.shape

#new = new.append(states_single_df_wide)
#new.shape
